...
*/
*/
class CControl implements Plugin{
	...
	public function init(){
		...
		$this->api->console->register('cc', "CControl commands", array($this, 'commands'));
		...
		switch($cmd){
			...
			case 'unmute':	
				if(isset($args[0])){
					...
					if($player === false){
						return "[CControl] That player doesn't exists";
					}
					$stmt = $this->db->prepare("SELECT * FROM players WHERE username = :name;");
					$stmt->bindValue(':name', $name, SQLITE3_TEXT);
					$result = $stmt->execute()->fetchArray(SQLITE3_ASSOC);
	...
	public function commands($cmd, $args, $issuer){
		...
		switch($args[0]){
			...
			case 'ban':
				if(!isset($args[1])){
					return '[CControl] Usage: /cc ban <player>';
				}
				$name = strtolower($args[1]);
				if($this->bannedPlayers->exists($name) === true){
					return '[CControl] Player already in the chat-ban-list';
				}
				$this->bannedPlayers->set($name);
				$this->bannedPlayers->save();
				return '[CControl] '.$name." can't chat anymore";
				break;
				
			case 'unban':		
				if(!isset($args[1])){
					return '[CControl] Usage: /cc unban <player>';
				}
				$name = strtolower($args[1]);
				if($this->bannedPlayers->exists($name) === false){
					return "[CControl] Player doesn't exists in the chat-ban-list";
				}
				$this->bannedPlayers->remove($name);
				$this->bannedPlayers->save();
				return '[CControl] '.$name.' can chat again';
				break;
								
			case 'reload':		
				$this->loadFiles();
				return '[CControl] Reloaded';
				break;
								
			case 'prefix':
			case 'suffix':		
				if (!isset($args[1]) or !isset($args[2])) {
					return '[CControl] Usage: /cc <prefix|suffix> <group> <text>';
				}
				$group = strtolower($args[1]);
				if(!isset($this->groups[$group])){
					return  "[CControl] The group doesn't exist";
				}
				$text = implode(array_slice($args, 2));
				if($args[0] === 'prefix'){
					$this->groups[$group]['prefix'] = $text;
					$output = '[CControl] The prefix of '.$group.' is changed to '.$text;
				}else{
					$this->groups[$group]['suffix'] = $text;
					$output = '[CControl] The suffix of '.$group.' is changed to '.$text;
				}
			...
			case 'mute':		
				$this->muted = true;
				return '[CControl] nobody can chat now';
				break;
								
			case 'unmute':		
				$this->muted = false;
				return '[CControl] un-muted the chat';
				break;
								
			default:			
				$output = "===============[CControl Commands]===============\n";
